...
using VideoConvertor.Utilities.Downloader;
...
public class BilibiliDownloader
{
    ...
    {
        var get = new BilibiliDownloader();
    ...
    private static Boolean biliVideo(string url,string? filename=null,string? dir=null)
    {
        ...
        DashVideo.Video? biliJson= JsonConvert.DeserializeObject<DashVideo.Video>(getBiliVideo_JSON(bv,getBiliVideo_Cid(bv)));
        if (biliJson.code == 0)
        {
            //清晰度索引
    ...
    public void GetInfomation(string url)
    {
        biliVideo(url,@".\test1.mp4");
        biliAudio(url,@".\test1.m4s");
        Downloaders down = new Downloaders();
